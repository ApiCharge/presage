...
impl<S: Store> Manager<S, Registered> {
                        ...
                        Some(Ok(Incoming::Envelope(envelope))) => {
                            ...
                            match envelope {
                                Ok(Some(content)) => {
                                    ...
                                    log::info!("[DIAG] open_envelope Ok(Some): mk={} skm={} seed={}", message_key.is_some(), sender_key_msg.is_some(), sender_key_seed.is_some());
                                ...
                                Ok(None) => {
                                    ...
                                    log::info!("[DIAG] open_envelope Ok(None): raw={} mk={} skdm_key={}", raw_content.is_some(), mk.is_some(), skdm_signing_key.is_some());
    ...
    /// Reserve a username with Signal's servers.
    /// Submits up to 20 username hashes; Signal returns the first available one.
    pub async fn reserve_username(
        &mut self,
        username_hashes: Vec<Vec<u8>>,
    ) -> Result<Vec<u8>, Error<S::Error>> {
        let mut ws = self.identified_websocket(false).await?;
        Ok(ws.reserve_username(&username_hashes).await?)
    }
    ...
    pub async fn confirm_username(
        ...
        let mut ws = self.identified_websocket(false).await?;
        Ok(ws.confirm_username(username_hash, zk_proof).await?)
    }
